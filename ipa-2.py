...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    if letter != ' ':
        shifted1 = (alphabet.index(letter) + shift) % 26
...
def caesar_cipher(message, shift):
    ...
    for letter in message:
            if message[place] != ' ':
                shifted1 = (alphabet.index(message[place]) + shift) % 26
                shifted2 = alphabet[shifted1]
...
def shift_by_letter(letter, letter_shift):
    ...
    if letter != ' ':
        shifted1 = (alphabet.index(letter) + alphabet.index(letter_shift)) % 26
        shifted2 = alphabet[shifted1]
        return shifted2
...
def scytale_cipher(message, shift):
    ...
    if len(message) % shift == 0:
        for i in range(len(message)):
            final_message = final_message + message[(i // shift) + (len(message) // shift) * (i % shift)]
    else:
...
def scytale_decipher(message, shift):
    ...
    add = 0
    for i in range(len(message)):
        dec_msg = dec_msg + message[(shift * place + add) % len(message)]
        place += 1
        if shift * place == len(message):
            add += 1
            place = 0
    return dec_msg
